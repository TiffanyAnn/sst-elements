...
srun -N2 -n32 -c2 sst ./emberLoad.py --model-options="--simConfig ${app}Sim --param merlin:merlinParams.rt_filename=${SCRATCH}/${app}_DF_${netSize}_rt.txt --param merlin:merlinParams.runtype=2 --param merlin:merlinParams.route=5" >> ${netSize}_LF_${app}_base.out
...
#echo $i
srun -N2 -n32 -c2 sst ./emberLoad1.py --model-options="--simConfig ${app}Sim --stats ${netSize}_${app}_${pct}percent_${i} --param merlin:merlinParams.rt_filename=${SCRATCH}/${app}_DF_${netSize}_rt.txt --param merlin:merlinParams.runtype=1 --param merlin:merlinParams.route=5 --param merlin:merlinParams.downPort_filename=${FILE}" >> ${netSize}_LF_${app}_${i}.out
